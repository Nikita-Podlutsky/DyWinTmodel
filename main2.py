...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, 
                epochs=20, patience=10, checkpoint_dir="checkpoints", vocab = None):
    """
    ...
    scaler = torch.amp.GradScaler(device)  # Автоматическое масштабирование градиентов
    ...
    optimizer, mode='min', factor=0.1, patience=patience, verbose=True, min_lr=1e-6, cooldown=1
    )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(tqdm.tqdm(train_loader, desc=f"Эпоха {epoch + 1}/{epochs} (обучение)")):
            ...
            wave, targets, target_lengths, input_lengths = wave.to(device), targets.to(device), target_lengths.to(device), input_lengths.to(device)
            optimizer.zero_grad()
            
            with torch.amp.autocast(str(device)):
                # Получение выходных данных модели
                outputs = model(wave.view(wave.shape[0],1,wave.shape[1]).float())  # Размер [batch_size, seq_length, vocab_size]
                ...
                # print(log_probs.shape, targets, input_lengths, target_lengths)
                # Вычисление потери
            ...
            scaler.step(optimizer)  # Обновляем веса
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(val_loader, desc=f"Эпоха {epoch + 1}/{epochs} (валидация)"):
                ...
                targets, target_lengths = prepare_targets_ctc(sent,vocab)
                wave, targets, target_lengths, input_lengths = wave.to(device), targets.to(device), target_lengths.to(device), input_lengths.to(device)
                outputs = model(wave.view(wave.shape[0],1,wave.shape[1]).float())
...
def run_training(sample_rate, n_fft, hop_length, n_mels, hid_dim, window_size, overlap_ratio, load_saved_model=False, model_path="checkpoints/best_model.pth", epochs = 20):
    """
    ...
    train_dataset = initialize_dataset(load_metadata=True, split="train", max_samples=100000000000)
    
    # Загрузка валидационного датасета
    val_dataset = initialize_dataset(load_metadata=True, split="validation", max_samples=500000000000)
    ...
    model = ConformerSpectrogramTransformer(sample_rate, n_fft, hop_length, n_mels, hid_dim, window_size, overlap_ratio, train_dataset.vocab_size).to(device)
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        # Инициализация модели
        model = ConformerSpectrogramTransformer(sample_rate, n_fft, hop_length, n_mels, hid_dim, window_size, overlap_ratio, dataset.vocab_size).to(device)
