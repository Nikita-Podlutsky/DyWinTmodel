...
class PositionalEncoding2D(nn.Module):
    """
    Positional encoding для двумерных изображений.
    ...
    def forward(self, x):
        ...
        height_enc = self._posenc(height_pos, self.dim)  # H, D
        width_enc = self._posenc(width_pos, self.dim)    # W, D
...
class ConvolutionModule(nn.Module):
    """Конволюционный модуль с гейтами для Conformer.
       Нормализация после свертки для сттабилизации градиентов
       activation: SiLU
       перенос входа на выход для предотвращения затухания градиентов
    """
...
class FeedForwardModule(nn.Module):
    """Модуль FFN для Conformer.
       прогоняем через лин слой для оценки всего окна
       дропаут =  0.1
    """
...
if __name__ == "__main__":
    ...
    model = ConformerSpectrogramTransformer(16000, n_fft, 160, n_mels, n_mfcc, hid_dim, window_size, overlap_ratio, vocab_size).to(device)
    model.eval()
    
    test_data = torch.randn(4,16000*20).to(device)
    import time
